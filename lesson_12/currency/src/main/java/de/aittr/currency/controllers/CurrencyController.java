...
public class CurrencyController {
    ...
    public ResponseEntity<?> getCurrency (@PathVariable(name = "code") String currencyCode ){
            try {
                return ResponseEntity.status(HttpStatus.OK).body(service.findByCode(currencyCode));
            } catch (Exception e) {
 ...
 */
    @PostMapping("/rates")
    public ResponseEntity<CurrencyResponseDto> addCurrency(@RequestBody Currency currency){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.createNewCurrency(currency));
    }
